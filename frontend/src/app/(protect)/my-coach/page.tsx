...
import { UserCheck, UserMinus, ShieldAlert, Activity, Heart, Calendar, Settings, X, Save, Edit3 } from 'lucide-react';
...
export default function MyCoachPage() {
  ...
  const openEditModal = () => {
    setEditPermissions({
      ...
      shareCalendar: coachLink.shareCalendar,
    });
    setIsModalOpen(true);
  };
    ...
    <div className="p-8 max-w-4xl mx-auto ml-64">
      ...
      {coachLink ? (
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-sm font-semibold transition-colors"
              >
          ...
          {/* Permissions Status (Visuel) */}
          <div className="mt-8 pt-6 border-t border-slate-100">
            <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Accès partagés :</h4>
            <div className="flex flex-wrap gap-2">
              {coachLink.shareActivities && <span className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-xs font-medium border border-indigo-100">Activitées</span>}
              {coachLink.shareCalendar && <span className="bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-xs font-medium border border-emerald-100">Calendrier</span>}
              {coachLink.sharePhysiology && <span className="bg-amber-50 text-amber-700 px-3 py-1 rounded-full text-xs font-medium border border-amber-100">Physiologie</span>}
            </div>
          </div>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-4">
              {[
                { key: 'shareActivities', icon: Activity, label: 'Activitées' },
                { key: 'shareCalendar', icon: Calendar, label: 'Calendrier' },
            ...
            <button 
              ...
              className="w-full mt-8 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-all"
            >
